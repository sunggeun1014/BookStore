document.addEventListener("DOMContentLoaded", function() {
  ...
  // 선택된 하위 메뉴 아이템 복원
  const selectedSubMenuItem = localStorage.getItem("selected-sub-menu-item");
  if (selectedSubMenuItem) {
    const subMenuItem = document.querySelector(`[data-sub-menu-id="${selectedSubMenuItem}"]`);
    if (subMenuItem) {
  ...
  menuItems.forEach((item, index) => {
    item.addEventListener("click", function(e) {
      ...
      // 현재 상태를 로컬 스토리지에 저장
      localStorage.setItem(`menu-open-${index}`, !isCurrentlyOpen);
  ...
  subMenuItems.forEach((subMenuItem) => {
    subMenuItem.addEventListener("click", function(e) {
      e.stopPropagation(); // 상위 메뉴 아이템의 클릭 이벤트 전파 방지

      // 모든 하위 메뉴 아이템에서 'selected' 클래스 제거
      subMenuItems.forEach((item) => {
        item.classList.remove("selected");
      });

      // 클릭된 하위 메뉴 아이템에 'selected' 클래스 추가
      this.classList.add("selected");

      // 선택된 하위 메뉴 아이템 ID를 로컬 스토리지에 저장
      localStorage.setItem("selected-sub-menu-item", this.getAttribute("data-sub-menu-id"));
    });
  });
});
